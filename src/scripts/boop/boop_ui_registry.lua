...
boop.registry.ui.helpTopics = boop.registry.ui.helpTopics or {
  {
    key = "start",
    title = "Start Here",
    summary = "A short path from installed package to a working hunt.",
    aliases = { "start", "gettingstarted", "intro", "basics", "general", "main", "home" },
    steps = {
      helpCommand("boop", "Open the home dashboard and read the current blocker/next action."),
      helpCommand("boop preset solo", "Apply the normal solo baseline: whitelist targeting, simple rage, autogold, and no party gating."),
      helpCommand("boop config targeting", "Choose how boop may pick targets in the current area."),
      helpCommand("boop whitelist", "Review or manage the names boop may attack in this area."),
      helpCommand("boop on", "Enable hunting once the target rules look right."),
      helpCommand("boop control", "Use the live control dashboard while hunting."),
    },
    commands = {
      ...
      helpCommand("boop help <goal>", "Open a focused help page such as `targeting`, `rage`, `party`, or `diagnostics`."),
    },
    notes = {
      "Help pages show a shortest useful path first. The full command reference lives below the first steps.",
  ...
  },
  {
    ...
    summary = "Where to look when you want current state, live controls, or guided settings.",
    aliases = { "control", "controls", "config", "settings", "dashboard" },
    steps = {
      helpCommand("boop", "Start here when you are unsure what boop is doing."),
      helpCommand("boop control", "Use this while hunting to see state, target, party, and movement controls."),
      helpCommand("boop config", "Use this when you want to change settings."),
    },
    commands = {
    ...
    notes = {
      "Status/control screens show live state. Help explains what the controls mean.",
  ...
  },
  {
    ...
    summary = "Recommended baseline for one-character hunting with whitelist targeting.",
    aliases = { "solo", "hunt", "hunting" },
    steps = {
      helpCommand("boop preset solo", "Set the normal solo baseline."),
      helpCommand("boop config targeting", "Confirm target mode and retarget behavior."),
      helpCommand("ih", "Run info-here, then use boop's clickable whitelist/blacklist actions for denizens."),
      helpCommand("boop whitelist", "Review what boop may attack in this area."),
      helpCommand("boop on", "Start hunting."),
    },
    commands = {
  ...
  },
  {
    ...
    summary = "How boop chooses denizens and how you manage whitelist/blacklist data.",
    aliases = { "targeting", "targets", "whitelist", "blacklist", "ih" },
    steps = {
      helpCommand("boop config targeting", "Open guided target controls."),
      helpCommand("boop targeting whitelist", "Attack only names on the current area's whitelist."),
      helpCommand("ih", "Capture denizens in the room and show clickable list actions."),
    ...
    commands = {
      ...
      helpCommand("boop blacklist global add <name>", "Block a denizen name everywhere."),
    },
    advanced = {
      ...
      helpCommand("boop set retargetOnPriority on|off", "Keep or ignore higher-priority mobs that enter while you already have a target."),
    ...
    notes = {
      "`manual` means boop will not choose targets. `auto` allows any valid denizen not blocked by blacklist rules.",
  ...
  },
  {
    ...
    summary = "How boop chooses attacks, rage actions, prequeue timing, and combat safety settings.",
    aliases = { "combat", "rage", "ragemode", "attackmode", "queue", "queueing", "prequeue", "flee", "pull", "focus" },
    steps = {
      helpCommand("boop config combat", "Open guided hunting, rage, queueing, interrupt, and safety controls."),
      helpCommand("boop ragemode", "Open the rage-mode chooser and see the current mode."),
      helpCommand("boop prequeue on", "Let boop queue standard attacks shortly before balance returns."),
      helpCommand("boop flee 30%", "Set the auto-flee threshold and keep auto-flee enabled."),
    },
    commands = {
      helpCommand("boop ragemode <simple|big|small|aff|tempo|combo|hybrid|none>", "Set how boop chooses battlerage actions."),
      helpCommand("boop prequeue on|off", "Enable or disable early standard-attack queueing."),
      helpCommand("boop lead <seconds>", "Set how early boop should prequeue before balance returns."),
      ...
      helpCommand("pull <mobname> <direction>", "Move, fire a damage battlerage attack at the typed mob name, and leap back."),
    },
    advanced = {
  ...
  },
  {
    ...
    steps = {
      helpCommand("diag", "Queue diagnose and pause attacks until the diagnose line plus prompt."),
      helpCommand("matic", "Queue `ldeck draw matic` and resume on prompt or timeout."),
      helpCommand("fly", "Queue `fly` and resume on prompt or timeout."),
    },
    commands = {
      helpCommand("diag", "Queue-clear, diagnose, and pause boop until diagnose completes or times out."),
      ...
      helpCommand("boop set diagTimeoutSeconds <seconds>", "Set the timeout used by diag and queued interrupt holds."),
    },
    notes = {
  ...
  },
  {
    key = "party",
    title = "Party & Leader",
    summary = "Follow a leader, call targets for others, coordinate walking, and inspect party combos.",
    aliases = { "party", "leader", "assist", "targetcall", "walk", "roster", "combos", "combo" },
    steps = {
      helpCommand("boop party", "Open the party dashboard and inspect current mode/leader/walker state."),
      helpCommand("boop assist <leader>", "Set the leader name before enabling assist or leader-call mode."),
      helpCommand("boop mode leader-call", "Follow a leader and wait for their called target id before attacking."),
  ...
  },
  {
    ...
    summary = "Use trip, session, area, target, ability, crit, and rage data to compare hunts.",
    aliases = { "stats", "trip", "records", "areas", "abilities", "crits", "compare" },
    steps = {
      helpCommand("boop stats", "Open the stats dashboard and suggested drill-downs."),
      helpCommand("boop trip start", "Start a deliberate trip bucket before a hunt."),
      helpCommand("boop stats compare", "Compare current trip to the last completed trip."),
    },
    commands = {
  ...
  },
  {
    ...
    summary = "Find out why boop made a decision, inspect state, adjust gagging, or use lower-level tools.",
    aliases = { "diagnostics", "debug", "trace", "gag", "advanced", "set", "get", "import", "foxhunt" },
    steps = {
      helpCommand("boop config debug", "Open guided diagnostics controls."),
      helpCommand("boop trace on", "Start recording recent decisions and GMCP events."),
      helpCommand("boop trace show", "Review recent decisions after the problem happens."),
      helpCommand("boop debug", "Show a current runtime-state snapshot."),
    },
    commands = {
