...
boop.registry.ui.helpTopics = boop.registry.ui.helpTopics or {
  {
    ...
    steps = {
      helpCommand("boop", "Open the home dashboard. Read `Blocker`, `systems`, `waits`, and `Next action`; they explain why boop will or will not attack."),
    ...
    commands = {
      helpCommand("boop", "Home dashboard: current state, target, blocker code, affected systems, waits-for state, and the best next screen."),
      helpCommand("boop control", "Live operations dashboard for hunting, movement, queueing, blocker status, and runtime state."),
      ...
      helpCommand("boop status", "Print the compact state line, including blocker `code -- label`, affected systems, and waits-for state."),
  ...
  },
  {
    ...
    steps = {
      helpCommand("boop", "Start here when you are unsure what boop is doing; the home dashboard shows blocker `code -- label` plus the next useful surface."),
      helpCommand("boop control", "Use this while hunting. It combines enabled state, target, blocker code, affected systems, waits-for state, queueing, party, walk, and quick navigation."),
    ...
    commands = {
      helpCommand("boop status", "Print a compact state summary with blocker `code -- label`, affected systems, and waits-for state."),
      helpCommand("boop control status", "Open the fuller status dashboard from the control route, including blocker systems and waits."),
  ...
  },
  {
    ...
    commands = {
      ...
      helpCommand("boop status", "Check enabled state, class, targeting mode, rage mode, and blocker `code -- label` in one line."),
      helpCommand("boop control", "Watch target, blocker code, queue, party, and walker state while hunting."),
  ...
  },
  {
    ...
    steps = {
      ...
      helpCommand("boop trace live on", "Stream newly collected entries for this session; collection must already be enabled."),
      helpCommand("boop trace show", "Review recent decisions after the problem happens."),
      helpCommand("boop debug", "Show a current runtime-state snapshot with blocker `code -- label`, systems, waits, target, class, balances, rage, and trace count."),
      helpCommand("boop debug attacks", "Check which profile, standard attacks, rage options, and preferences are currently loaded."),
    },
    commands = {
      helpCommand("boop debug", "Show the debug snapshot for current runtime state, including blocker systems and waits-for state."),
      ...
      helpCommand("boop trace live on|off", "Toggle session-only live streaming without enabling or disabling trace collection."),
    ...
    notes = {
      "Use trace when you need decision-flow history; use status/debug when you need the current blocker code, affected systems, and waits-for state.",
