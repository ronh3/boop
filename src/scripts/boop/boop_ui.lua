...
function boop.ui.debugSkills()
  local processing = boop.skills and boop.skills.processing or false
  local pendingCount = boop.skills and boop.skills.pending and tostring(#(function()
    local t = {}
    for k, _ in pairs(boop.skills.pending) do t[#t + 1] = k end
    return t
  end)()) or "0"
  ...
  boop.util.echo(string.format(
    "debug skills | processing:%s | pending:%s | lastList group:%s count:%s | lastInfo skill:%s group:%s",
    tostring(processing), tostring(pendingCount), tostring(lastListGroup), tostring(lastListCount),
