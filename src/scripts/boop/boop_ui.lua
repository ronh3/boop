...
local function sortedEnabledKeys(map)
  local keys = {}
...
local function makeBlockerDetails(
  ...
  return {
    ...
    text = text,
    nextAction = tostring(nextAction or "let boop attack"),
    systems = systems or {},
    waitsFor = waitsFor or {},
    systemsText = blockerKeyText(systems),
...
local function currentBlockerDetails()
  ...
  if tostring(eligibleDenizen or "") == "" then
    ...
    if boop.walk and boop.walk.isAvailable and not boop.walk.isAvailable() then
      return makeBlockerDetails("walk_missing", "walk package missing", "boop walk install", { walk = true }, { manual = true }, "ui")
    end
...
currentBlocker = function()
  local details = currentBlockerDetails()
  return details.text, details.nextAction, details
end

local function blockerColor(details)
  return details and details.active and "yellow" or "green"
end
...
blockerDetailText = function(details)
  ...
  local parts = {}
  if details.systemsText and details.systemsText ~= "" and details.systemsText ~= "none" then
    parts[#parts + 1] = "systems: " .. details.systemsText
  end
  if details.waitsText and details.waitsText ~= "" and details.waitsText ~= "none" then
...
local function printBlockerDetailRows(details, startRow)
  local row = tonumber(startRow) or 1
  if details and details.systemsText and details.systemsText ~= "none" then
    uiPrintRow(row, "Affected systems", details.systemsText, "cyan")
    row = row + 1
  end
  if details and details.waitsText and details.waitsText ~= "none" then
    uiPrintRow(row, "Waits for", details.waitsText, "cyan")
    row = row + 1
  end
  return row
end
...
local function renderStatusDashboard()
  ...
  if cecho then
    ...
    row = row + 1
    row = printBlockerDetailRows(blockerDetails, row)
...
function boop.ui.leap(direction)
  ...
  if dir == "" then
    boop.util.warn("leap needs a direction")
    boop.util.info("Usage: leap <direction>")
    return
  end
...
local function configRenderCombatSection()
  ...
  if cecho then
    ...
    uiPrintSection("settings")
    uiPrintToggleControl(1, "Hunting", boop.config.enabled, function()
      boop.ui.config("combat 1")
    end, "Toggle hunting enabled")
    uiPrintActionControl(2, "Rage mode", tostring(boop.config.attackMode or "simple"), "yellow", "[set]", "info", function()
      boop.ui.config("combat 2")
    end, "Open ragemode menu")
    uiPrintActionControl(3, "Diag", "ready", "yellow", "[run]", "info", function()
      boop.ui.config("combat 3")
    end, "Queue diagnose and pause attacks")
    uiPrintToggleControl(4, "Queueing", not not boop.config.useQueueing, function()
      boop.ui.config("combat 4")
    end, "Toggle queueing mode")
    uiPrintToggleControl(5, "Prequeue", not not boop.config.prequeueEnabled, function()
      boop.ui.config("combat 5")
    end, "Toggle prequeue")
    uiPrintActionControl(6, "Attack lead", string.format("%.2fs", lead), "yellow", "[set]", "info", function()
      boop.ui.config("combat 6")
    end, "Prepare boop lead command")
    uiPrintActionControl(7, "Diag timeout", string.format("%.2fs", diagTimeout), "yellow", "[set]", "info", function()
      boop.ui.config("combat 7")
    end, "Prepare boop set diagtimeout command")
    uiPrintActionControl(8, "Tempo window", string.format("%.1fs", tempoWindow), "yellow", "[set]", "info", function()
      boop.ui.config("combat 8")
    end, "Prepare boop set tempoRageWindowSeconds command")
    uiPrintActionControl(9, "Tempo squeeze ETA", string.format("%.2fs", tempoEta), "yellow", "[set]", "info", function()
      boop.ui.config("combat 9")
    end, "Prepare boop set tempoSqueezeEtaSeconds command")
    uiPrintActionControl(10, "Assist leader", assistStatusText(), boop.config.assistEnabled and "green" or "yellow", "[set]", "info", function()
    ...
    uiPrintActionControl(15, "Flee at", fleeShown, boop.config.fleeEnabled and "yellow" or "red", "[set]", "info", function()
      boop.ui.config("combat 15")
    end, "Prepare boop flee command")
    uiPrintActionControl(16, "Focus verb", tostring(boop.config.focusVerb or "speed"), "yellow", "[set]", "info", function()
  ...
  boop.util.echo("[2] Rage mode                [ " .. tostring(boop.config.attackMode or "simple") .. " ] [set]")
  boop.util.echo("[3] Diag                     [ ready ] [run]")
  boop.util.echo("[4] Queueing                 [ " .. boolText(not not boop.config.useQueueing) .. " ] [toggle]")
  boop.util.echo("[5] Prequeue                 [ " .. boolText(not not boop.config.prequeueEnabled) .. " ] [toggle]")
  boop.util.echo(string.format("[6] Attack lead              [ %.2fs ] [set]", lead))
  boop.util.echo(string.format("[7] Diag timeout             [ %.2fs ] [set]", diagTimeout))
  boop.util.echo(string.format("[8] Tempo window             [ %.1fs ] [set]", tempoWindow))
...
local function configRenderTargetingSection()
  ...
  if cecho then
    ...
    uiPrintRow(nil, "Called target", calledId, "cyan")
    uiPrintRow(nil, "Room denizens", tostring(denizenCount), "cyan")
...
local function configRenderLootSection()
  ...
  if cecho then
    ...
    uiPrintSection("live")
    boop.util.echo("Queue: " .. pending .. " | pack: " .. shownPack)

    uiPrintSection("settings")
    uiPrintToggleControl(1, "Auto grab", not not boop.config.autoGrabGold, function()
      boop.ui.config("loot 1")
    end, "Toggle automatic sovereign pickup")
    uiPrintActionControl(2, "Gold pack", shownPack, "yellow", "[set]", "info", function()
      boop.ui.config("loot 2")
    end, "Prepare boop pack command")
    uiPrintActionControl(3, "Clear pack", shownPack, pack ~= "" and "yellow" or "text", "[clear]", pack ~= "" and "err" or "text", function()
      boop.ui.config("loot 3")
    end, "Disable auto stashing")
    uiPrintActionControl(4, "Pack test", shownPack, pack ~= "" and "yellow" or "text", "[run]", "info", function()
  ...
  boop.util.echo("[1] Auto grab                [ " .. boolText(not not boop.config.autoGrabGold) .. " ] [toggle]")
  boop.util.echo("[2] Gold pack                [ " .. shownPack .. " ] [set]")
  boop.util.echo("[3] Clear pack               [ " .. shownPack .. " ] [clear]")
  boop.util.echo("[4] Pack test                [ " .. shownPack .. " ] [run]")
  boop.util.echo("----------------------------------------")
