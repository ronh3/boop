...
describe("boop config and list persistence paths", function()
  ...
  it("persists canonicalized targeting, target-call gating, gold-pack, rage aff callout, and theme edits, but keeps party size session-local", function()
    ...
    assert.are.same({ key = "uiTheme", value = "ocean" }, saved_configs[5])
    assert.are.same({ delete = "partySize" }, saved_configs[6])
