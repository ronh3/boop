...
describe("boop ui home", function()
  ...
  it("shows a rewritten help home with guided entry points", function()
    ...
    assert.is_true(joined:find("[1] Start Here -> A short path from installed package to a working hunt.", 1, true) ~= nil)
    assert.is_true(joined:find("[4] Targeting & Lists -> How boop chooses denizens and how you manage whitelist/blacklist data.", 1, true) ~= nil)
  ...
  it("shows a rewritten workflow help topic with first steps, commands, and notes", function()
    ...
    assert.is_true(joined:find("Use trip, session, area, target, ability, crit, and rage data to compare hunts.", 1, true) ~= nil)
  ...
  it("adds rich hover descriptions to help command rows", function()
    ...
    assert.is_true(joined:find("Open the party dashboard and inspect current mode/leader/walker state%.") ~= nil)
