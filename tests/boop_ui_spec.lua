...
describe("boop ui home", function()
  ...
  it("shows a dedicated control dashboard", function()
    ...
    assert.is_true(echoes[5]:find("Party: assist ON -> Leader | targetcall ON | size 3 | walk OFF | theme occultist", 1, true) ~= nil)
  ...
  it("shows a consolidated party dashboard and separate roster manager", function()
    ...
    assert.is_true(echoes[5]:find("Movement: walk OFF | blocker waiting for leader target call", 1, true) ~= nil)
    ...
    assert.are.equal("Quick: boop party assist <leader> | boop party targetcall on|off | boop party affcalls on|off | boop party walk | boop roster | boop combos", echoes[9])
  end)
