...
describe("boop wield tracking", function()
  ...
  it("tracks left and right wielded items from the inventory list attrib flags", function()
    ...
    assert.are.equal("a broad-bladed sword of hardship", boop.state.wieldedLeft.name)
    assert.are.equal("12", boop.state.wieldedLeft.id)
    assert.are.equal("a curved blade of broken chains", boop.state.wieldedRight.name)
    assert.are.equal("11", boop.state.wieldedRight.id)
  end)
  ...
  it("updates and clears tracked wielded items from inventory update and remove events", function()
    ...
    boop.onItemsUpdate()
    assert.is_false(boop.state.wieldedRight)
    ...
    assert.is_false(boop.state.wieldedLeft)
  end)
