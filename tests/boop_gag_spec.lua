...
describe("boop gag summaries", function()
  ...
  it("condenses an own attack into one summary line with damage, crit, and balance", function()
    ...
    assert.is_true(outputs[1]:find("Total: 1234", 1, true) ~= nil)
  ...
  it("labels same-target proc damage inside the compact attack summary", function()
    ...
    assert.is_true(outputs[1]:find("Total: 2630", 1, true) ~= nil)
  ...
  it("condenses a Blue Dragon blast and gear proc on equilibrium used", function()
    ...
    assert.is_true(outputs[1]:find("Total: 3750", 1, true) ~= nil)
  ...
  it("condenses two pre-balance self jab-shaped hits as one DSL summary", function()
    ...
    assert.is_true(outputs[1]:find("3210 physical cutting - crits: 8xCRIT", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Total: 3210", 1, true) ~= nil)
  ...
  it("suppresses hyena maul flavor and preserves the following DSL summary", function()
    ...
    assert.is_true(outputs[2]:find("3210 physical cutting - crits: 8xCRIT", 1, true) ~= nil)
    assert.is_true(outputs[2]:find("Total: 3210", 1, true) ~= nil)
  ...
  it("labels companion maul and paired doublewhirl damage in one summary", function()
    ...
    assert.is_true(outputs[1]:find("Gear: 63 psychic - crits: 2xCRIT", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Doublewhirl: 1740 physical blunt", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Total: 3369", 1, true) ~= nil)
  ...
  it("keeps repeated doublewhirl hits sourced after gear procs", function()
    ...
    assert.is_true(outputs[1]:find("Doublewhirl: 1300 physical blunt", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Gear: 306 psychic - crits: 16xCRIT", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Gear: 650 physical blunt", 1, true) == nil)
    assert.is_true(outputs[1]:find("Total: 1606", 1, true) ~= nil)
    assert.is_true(outputs[1]:find("Bal: 3.1s", 1, true) ~= nil)
  end)
