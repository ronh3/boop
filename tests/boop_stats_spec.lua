...
describe("boop stats", function()
  ...
  it("shows a concise stats help page", function()
    ...
    assert.are.equal("  Optimize", messages[12])
    assert.are.equal("    boop stats areas <scope> <limit> <metric>     -> rank by gold, xp, goldhr, xphr, ttk", messages[14])
    assert.are.equal("  Compare and maintain", messages[21])
    assert.are.equal("    boop stats compare                            -> compare trip vs lasttrip", messages[22])
    assert.are.equal("    boop stats compare <left> <right>             -> compare two explicit scopes", messages[23])
    assert.are.equal("    boop help stats                               -> workflow help for stats", messages[25])
  end)
