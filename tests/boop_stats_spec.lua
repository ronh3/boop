...
describe("boop stats", function()
  ...
  it("shows a concise stats help page", function()
    ...
    assert.are.equal("  Start", messages[2])
    assert.are.equal("    boop stats                 -> dashboard and suggested drill-downs", messages[3])
    assert.are.equal("  Totals", messages[6])
    assert.are.equal("    boop stats login           -> current-login totals across boop toggles", messages[8])
    assert.are.equal("    boop stats show <scope>    -> explicit route for a named totals scope", messages[12])
    assert.are.equal("  Optimize", messages[13])
    assert.are.equal("    boop stats areas <scope> <limit> <metric>     -> rank by gold, xp, goldhr, xphr, ttk", messages[15])
    assert.are.equal("  Compare and maintain", messages[22])
    assert.are.equal("    boop stats compare                            -> compare trip vs lasttrip", messages[23])
    assert.are.equal("    boop stats compare <left> <right>             -> compare two explicit scopes", messages[24])
    assert.are.equal("    boop help stats                               -> workflow help for stats", messages[26])
  end)
  ...
  it("shows a summary-first dashboard with optimization next views", function()
    ...
    assert.is_true(messages[6]:find("best session area: Mhaldor | 120 kills/hr | 540000 xp/hr", 1, true) ~= nil)
    ...
    assert.is_true(messages[8]:find("top session ability: warp | kills 5 | avg dmg 120 | crit 25%", 1, true) ~= nil)
  ...
  it("falls back to lifetime guidance when session and trip are empty", function()
    ...
    assert.is_true(messages[6]:find("best lifetime area: Manara Burrow | 300 kills/hr | 12000000 xp/hr", 1, true) ~= nil)
    assert.are.equal("  top lifetime target: (none yet)", messages[7])
    assert.is_true(messages[8]:find("top lifetime ability: slaughter | kills 260 | avg dmg 145 | crit 10.1%", 1, true) ~= nil)
